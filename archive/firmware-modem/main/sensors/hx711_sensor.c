...
static const uint32_t HX711_LOG_INTERVAL_MS = MODEM_HX711_POLL_INTERVAL_MS;
static const size_t HX711_SAMPLES = MODEM_HX711_AVG_SAMPLES;
static const uint32_t HX711_WAIT_TIMEOUT_MS = MODEM_HX711_READY_TIMEOUT_MS;
static const uint32_t HX711_WAIT_BACKOFF_MS = MODEM_HX711_READY_BACKOFF_MS;
static const uint32_t HX711_WAIT_RETRY = MODEM_HX711_READY_RETRY_COUNT;
...
static esp_err_t hx711_apply_gain(hx711_gain_t gain)
{
    ...
    if (err == ESP_OK) {
        // Give the HX711 Zeit, nach dem Gain-Umschalten stabil zu werden
        vTaskDelay(pdMS_TO_TICKS(50));

        // Dummy-Read nach optionalen Re-Trys auf „ready“
        int32_t dummy = 0;
                               ...
                               int32_t *raw_out, float *kg_out, bool *ok_out)
{
    ...
    *kg_out = 0.0f;
    ...
    for (size_t i = 0; i < HX711_SAMPLES; i++) {
        ...
        for (uint32_t r = 0; r <= HX711_WAIT_RETRY; r++) {
            ...
            wait_us = esp_timer_get_time() - t_before_wait;
            if (err == ESP_OK) {
                wait_err = ESP_OK;
                break;
            }
        ...
        if (wait_err != ESP_OK) {
            ESP_LOGW(TAG, "%s: wait timeout nach %lld us (sample %u)", label, (long long)wait_us, (unsigned)i);
            return;
        }
        ...
        sum += raw_sample;
        ESP_LOGI(TAG, "%s: sample %u raw=%ld wait=%lld us", label, (unsigned)i, (long)raw_sample, (long long)wait_us);
    }
    ...
    *ok_out = true;
    ...
    ESP_LOGI(TAG, "%s: avg=%ld (%.2f kg) samples=%u dur=%lld us", label, (long)avg, kg,
             (unsigned)HX711_SAMPLES, (long long)(esp_timer_get_time() - t_start));
}
    ...
    while (1) {
        ...
        if (xSemaphoreTake(s_mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
            hx711_read_channel(HX711_GAIN_A_128, "HX711 A",
                                MODEM_HX711_OFFSET_A, MODEM_HX711_SCALE_A,
                                &raw_a, &kg_a, &ok_a);
            if (MODEM_HX711_ENABLE_CHANNEL_B) {
                hx711_read_channel(HX711_GAIN_B_32, "HX711 B",
                                    MODEM_HX711_OFFSET_B, MODEM_HX711_SCALE_B,
                                    &raw_b, &kg_b, &ok_b);
            }
...
esp_err_t hx711_app_start(void)
{
    ...
    s_hx.dout = MODEM_HX711_DOUT_GPIO;
    s_hx.pd_sck = MODEM_HX711_SCK_GPIO;
    ...
    gpio_set_pull_mode(MODEM_HX711_DOUT_GPIO, GPIO_PULLUP_ONLY);

    // Präsenz-Check mit Re-Trys und Backoff, erst nach Startup-Delay
    vTaskDelay(pdMS_TO_TICKS(MODEM_HX711_STARTUP_DELAY_MS));
    ...
    if (pres_err != ESP_OK) {
        ...
        // Nicht abbrechen – ggf. wird der Sensor kurz danach aktiv
    }
    ...
    ESP_LOGI(TAG, "HX711 gestartet (DOUT=%d, SCK=%d)", MODEM_HX711_DOUT_GPIO, MODEM_HX711_SCK_GPIO);
    return ESP_OK;
}
