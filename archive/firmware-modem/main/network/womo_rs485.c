...
static const uart_port_t s_uart_port = MODEM_RS485_UART_PORT;
...
esp_err_t womo_rs485_init(void)
{
    ...
    uart_config_t uart_cfg = {
        .baud_rate = MODEM_RS485_BAUDRATE,
    ...
    gpio_set_pull_mode(MODEM_RS485_DE_GPIO, GPIO_PULLDOWN_ONLY);
    gpio_set_direction(MODEM_RS485_DE_GPIO, GPIO_MODE_OUTPUT);
    gpio_set_level(MODEM_RS485_DE_GPIO, 0);

    err = uart_set_pin(s_uart_port,
                       MODEM_RS485_TX_GPIO,
                       MODEM_RS485_RX_GPIO,
                       MODEM_RS485_RTS_GPIO,
                       UART_PIN_NO_CHANGE);
    if (err != ESP_OK) {
    ...
    uint8_t rx_timeout = (uint8_t)(MODEM_RS485_READ_TIMEOUT_MS / 10);
    if (rx_timeout == 0) {
        rx_timeout = 1;
    }
    ...
    s_initialized = true;
    ESP_LOGI(TAG, "RS485 UART initialized (UART%d, %d baud)", (int)s_uart_port, MODEM_RS485_BAUDRATE);
    return ESP_OK;
}
