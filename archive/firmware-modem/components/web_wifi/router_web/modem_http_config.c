...
static modem_power_status_provider_t s_power_provider = NULL;
static modem_imu_fast_request_cb_t s_imu_fast_cb = NULL;
...
static esp_err_t system_station_get_handler(httpd_req_t *req)
{
    ...
    if (at) {
        if (at_cmd_get_signal_quality(at, &csq) == ESP_OK && csq.rssi != 99) {
            csq_dbm = -113 + 2 * csq.rssi;
        }
    }
    struct modem_netif_sta_info *node;
    bool first = true;
    ...
    httpd_resp_sendstr_chunk(req, "{\"station_list\":[");

    SLIST_FOREACH(node, &s_sta_list_head, field) {
        int len = snprintf(buf, sizeof(buf), "%s{\"name_str\":\"%s\",\"mac_str\":\"" MACSTR "\",\"ip_str\":\"" IPSTR "\",\"online_time_s\":\"%lld\"}",
                            first ? "" : ",",
                            node->name,
                            MAC2STR(node->mac),
                            IP2STR(&node->ip),
        ...
        if (len < 0 || len >= (int)sizeof(buf)) {
            ESP_LOGW(TAG, "station json truncated");
            buf[sizeof(buf) - 2] = '}';
            buf[sizeof(buf) - 1] = '\0';
        }
    ...
    int len = snprintf(buf, sizeof(buf), "],\"signal\":{\"rssi\":%d,\"ber\":%d,\"dbm\":%d},\"now_time\":\"%lld\"}",
                      csq.rssi,
                      csq.ber,
                      csq_dbm,
                      esp_timer_get_time());
    ...
    httpd_resp_sendstr_chunk(req, NULL); // Terminator
...
static esp_err_t system_power_get_handler(httpd_req_t *req)
{
    esp_err_t err = basic_auth_get(req);
    REST_CHECK(err == ESP_OK, "not login yet", end);
    ...
    modem_power_status_t snap = {0};
    esp_err_t res = s_power_provider ? s_power_provider(&snap) : ESP_ERR_NOT_SUPPORTED;

    double age_s = 0.0;
    if (snap.timestamp_us > 0) {
        int64_t now = esp_timer_get_time();
        age_s = (double)(now - snap.timestamp_us) / 1000000.0;
        if (age_s < 0) {
            age_s = 0.0;
        }
    }
    ...
    char buf[192];
    if (res == ESP_OK && snap.valid) {
        int len = snprintf(buf, sizeof(buf), "{\"ok\":true,\"voltage\":%.3f,\"percent\":%.1f,\"age_s\":%.2f}",
                           snap.voltage, snap.percent, age_s);
        httpd_resp_send(req, buf, (len > 0) ? len : HTTPD_RESP_USE_STRLEN);
    } else if (s_power_provider == NULL) {
        const char *msg = "{\"ok\":false,\"msg\":\"kein Provider\"}";
        httpd_resp_send(req, msg, HTTPD_RESP_USE_STRLEN);
    } else {
        const char *msg = "{\"ok\":false,\"msg\":\"Messung fehlgeschlagen\"}";
        httpd_resp_send(req, msg, HTTPD_RESP_USE_STRLEN);
    }
...
static esp_err_t rest_common_get_handler(httpd_req_t *req)
{
    ...
    // Root → Horizon; /status → bisheriges index.html
    if (strcmp(req->uri, "/") == 0) {
        strlcat(filepath, "/horizon.html", sizeof(filepath));
    ...
    httpd_resp_set_hdr(req, "Cache-Control", "public, max-age=600, stale-while-revalidate=60");
...
static httpd_uri_t system_power_get = {
    .uri = "/system/power_state",
    .method = HTTP_GET,
    .handler = system_power_get_handler,
    .user_ctx = NULL
};
...
// Platzhalter-IMU-API für den Horizont
static esp_err_t imu_get_handler(httpd_req_t *req)
{
    httpd_resp_set_type(req, "application/json");
    httpd_resp_set_hdr(req, "Cache-Control", "no-store, max-age=0");
    if (s_imu_fast_cb) {
        s_imu_fast_cb(2000, 500);
    }
    web_wifi_imu_snapshot_t snap = {0};
    bool ok = web_wifi_imu_get_snapshot(&snap);
    if (!ok) {
        const char *payload = "{\"ok\":false,\"msg\":\"IMU nicht verfuegbar\"}";
        return httpd_resp_send(req, payload, HTTPD_RESP_USE_STRLEN);
    }

    char payload[256] = {0};
    const char *calibrated = snap.calibrated ? "true" : "false";
    int len = snprintf(payload, sizeof(payload),
                       "{\"ok\":true,\"yaw_deg\":%.2f,\"roll_deg\":%.2f,\"pitch_deg\":%.2f,"
                       "\"cal\":[%u,%u,%u,%u],\"calibrated\":%s}",
                       snap.yaw_deg,
                       snap.roll_deg,
                       snap.pitch_deg,
                       snap.cal_sys,
    ...
    if (len <= 0 || len >= (int)sizeof(payload)) {
        const char *fallback = "{\"ok\":false,\"msg\":\"IMU payload zu gross\"}";
        return httpd_resp_send(req, fallback, HTTPD_RESP_USE_STRLEN);
    }
    return httpd_resp_send(req, payload, HTTPD_RESP_USE_STRLEN);
}
...
static httpd_handle_t start_webserver(const char *base_path)
{
    ...
    // Mehr gleichzeitige Verbindungen und Backlog zulassen (Grenze 7 wegen LWIP_MAX_SOCKETS)
    config.max_open_sockets = 7;    // default 7, nicht höher ohne LWIP_MAX_SOCKETS anzupassen
    config.backlog_conn = 10;       // default 5
    // Bump Server-Task: mehr Stack & höhere Prio, um lange Antwortzeiten zu vermeiden
    config.stack_size = 8 * 1024;      // Default 4096
    config.task_priority = 5;          // Default 4
    // TCP-Puffer im httpd erhöhen
    config.recv_wait_timeout = 10;     // Sekunden
    config.send_wait_timeout = 10;     // Sekunden
    // Keep-Alive aktivieren, damit Folge-Requests schneller sind (Tabswitch, Assets)
    ...
    ESP_LOGI(TAG, "Starting server on port: '%d' (stack=%d, prio=%d)", config.server_port, config.stack_size, config.task_priority);
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        system_power_get.user_ctx = ctx_info;
        httpd_register_uri_handler(server, &system_power_get);
