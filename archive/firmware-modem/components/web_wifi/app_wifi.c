...
void wifi_init_softap(esp_netif_t *wifi_netif, const modem_wifi_config_t *config)
{
    ...
    // Setze Länderprofil (DE/EU) und Kanalpolicy fix, damit alle Kanäle 1-13 erlaubt sind
    ...
    snprintf((char*)wifi_config.ap.ssid, sizeof(wifi_config.ap.ssid), "%s", config->ap_ssid);
    wifi_config.ap.ssid_len = strlen((char*)wifi_config.ap.ssid);
    snprintf((char*)wifi_config.ap.password, sizeof(wifi_config.ap.password), "%s", config->ap_password);
    wifi_config.ap.max_connection = config->max_connection;
    wifi_config.ap.authmode = config->authmode;
    wifi_config.ap.ssid_hidden = config->ssid_hidden;
    wifi_config.ap.pmf_cfg.capable = true;
    wifi_config.ap.pmf_cfg.required = false; // Handy-Kompatibilität: PMF nur optional
    wifi_config.ap.pairwise_cipher = WIFI_CIPHER_TYPE_CCMP; // Nur CCMP, kein TKIP
    ...
    ESP_LOGI(TAG, "wifi_init_softap finished.SSID:%s password:%s ch=%d auth=%d bw=%d hidden=%d",
             config->ap_ssid, config->ap_password, (int)wifi_config.ap.channel,
             (int)wifi_config.ap.authmode, (int)config->bandwidth, (int)config->ssid_hidden);
}
