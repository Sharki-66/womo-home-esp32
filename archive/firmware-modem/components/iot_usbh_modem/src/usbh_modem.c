...
static void _modem_daemon_task(void *param)
{
    ...
    while (true) {
        ...
        case STAGE_START_PPP: {
            ...
            if (result.rssi != 99) {
                int csq_dbm = -113 + 2 * result.rssi;
                ESP_LOGI(TAG, "Signal OK: CSQ=%d (%d dBm), BER=%d", result.rssi, csq_dbm, result.ber);
            } else {
                ESP_LOGW(TAG, "Signal: CSQ=99 (unbekannt), BER=%d", result.ber);
            }
