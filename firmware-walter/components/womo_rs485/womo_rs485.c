...
static const uart_port_t s_uart_port = WALTER_RS485_UART_PORT;
...
esp_err_t womo_rs485_init(void)
{
    ...
    .baud_rate = WALTER_RS485_BAUDRATE,
    ...
    uint8_t rx_timeout = (uint8_t)(WALTER_RS485_READ_TIMEOUT_MS / 10);
    if (rx_timeout == 0) {
        rx_timeout = 1;
    }
    ...
    s_initialized = true;
    ESP_LOGI(TAG, "RS485 UART initialized (UART%d, %d baud)", (int)s_uart_port, WALTER_RS485_BAUDRATE);
    return ESP_OK;
}
...
int womo_rs485_read(uint8_t *buffer, size_t max_length, TickType_t ticks_to_wait)
{
    ...
    int bytes_read = uart_read_bytes(s_uart_port,
                                     buffer,
                                     max_length,
                                     ticks_to_wait);
    return bytes_read;
