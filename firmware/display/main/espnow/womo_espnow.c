...
/*
 * ESP-NOW Transport Layer – Display
 *
 * Spiegelschicht zum Sensorboard-Transport: sendet Kommandos ans Sensorboard,
 * empfängt Sensor-JSON-Frames. Voll-Duplex, keine Half-Duplex-Kollisionsgefahr.
 *
 * Peer-Discovery: Display sendet zunächst per Broadcast.
 * Sobald das Sensorboard antwortet, wird seine MAC-Adresse gelernt
...
static const char *TAG = "womo_espnow";
...
static womo_espnow_recv_cb_t s_recv_cb    = NULL;
                           ...
                           const uint8_t *data, int len)
{
    ...
    // Sensorboard-MAC beim ersten empfangenen Paket lernen
    ...
    if (is_new) {
        ...
        ESP_LOGI(TAG, "Sensorboard-Peer gelernt: " MACSTR, MAC2STR(recv_info->src_addr));
    }
...
esp_err_t womo_espnow_init(void)
{
    ...
    // Broadcast-Peer für Discovery (solange Sensorboard-MAC unbekannt)
    ...
    ESP_LOGI(TAG, "ESP-NOW Display Transport bereit");
    return ESP_OK;
}
