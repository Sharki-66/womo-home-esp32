/*
 * MIGRATION DRAFT – LVGL v8 → v9
 *
 * Diese Datei ist NICHT Teil des aktuellen Builds (nicht in CMakeLists.txt).
 * Sie dient als Referenz/Entwurf für die geplante LVGL v9 Migration.
 * Details: .github/lvgl-v8-to-v9-migration.md
 *
 * Bekannte offene Punkte in diesem Entwurf:
 *  - CH422G-Backlight-Init muss einen gemeinsamen I2C-Bus-Handle zurückgeben
 *  - Touch-Initialisierung muss nach I2C-Bus-Erstellung erfolgen
 *  - LVGL-Port-Task-Loop muss mit FreeRTOS-Task und lvgl_port_lock() abgesichert werden
 */
...
static void lvgl_touch_read_cb(lv_indev_t *indev __attribute__((unused)), lv_indev_data_t *data)
{
    uint16_t touch_x[1];
    uint16_t touch_y[1];
...
static i2c_master_bus_handle_t init_i2c_and_backlight(void)
{
...
static void init_touch(i2c_master_bus_handle_t bus_handle)
{
...
void app_main(void)
{
    // 1. I2C Bus + Backlight initialisieren
    i2c_master_bus_handle_t bus_handle = init_i2c_and_backlight();

    // 2. Touch Hardware initialisieren
    init_touch(bus_handle);

    // 3. RGB Panel konfigurieren
    ...
    // 4. LVGL 9 Initialisierung
    lv_init();
    lv_display_t *disp = lv_display_create(LCD_H_RES, LCD_V_RES);

    // Buffer im PSRAM (Teil-Update)
    ...
    // 5. Touch als LVGL Input Device registrieren
    lv_indev_t *indev = lv_indev_create();
    lv_indev_set_type(indev, LV_INDEV_TYPE_POINTER);
    lv_indev_set_read_cb(indev, lvgl_touch_read_cb);

    // 6. EEZ Studio UI starten
    ui_init();
