...
typedef struct {
    bool powered;          //!< GNSS-Modul eingeschaltet
    bool fix_valid;        //!< Fix vorhanden (fix_status > 0)
    int run_status;        //!< <1>=running, <0>=stopped
    int fix_status;        //!< 0=no fix, 1=2D/3D, 2=Diff
    time_t utc_time;       //!< UTC-Zeit des Fixes (epoch, 0 bei unbekannt)
    double latitude;       //!< Breite in Grad
    double longitude;      //!< Länge in Grad
    double altitude_m;     //!< Höhe in Metern
    double speed_kph;      //!< Geschwindigkeit in km/h (laut SIMCOM-Doku)
    double course_deg;     //!< Kurs über Grund in Grad
    double hdop;           //!< Horizontal DOP, 0 wenn nicht angegeben
    int sats_in_view;      //!< Sichtbare Satelliten
    int sats_in_use;       //!< Genutzte Satelliten
} esp_modem_at_gnss_info_t;
...
/**
 * @brief GNSS einschalten oder abschalten (SIMCOM neu: AT+CGNSSPWR, alt: AT+CGNSPWR)
 */
esp_err_t at_cmd_gnss_power(at_handle_t at_handle, bool enable);

/**
 * @brief GNSS einschalten/abschalten (A7670E: AT+CGNSSPWR)
 */
esp_err_t at_cmd_cgnss_power(at_handle_t at_handle, bool enable);

/**
 * @brief GNSS-Modus setzen (z.B. GPS only =1, GNSS mix =3 je nach Modul)
 */
esp_err_t at_cmd_cgnss_mode(at_handle_t at_handle, int mode);

/**
 * @brief GNSS-Status/Fix abfragen (SIMCOM: AT+CGNSINF)
 */
esp_err_t at_cmd_gnss_get_info(at_handle_t at_handle, esp_modem_at_gnss_info_t *info);

/**
 * @brief Fallback für SIMCOM-Varianten mit AT+CGPS (SIM7600/ältere A76xx)
 */
...
/**
 * @brief GNSS-Detailabfrage (A7670E: AT+CGNSSINFO, liefert Sats/HDOP)
 */
